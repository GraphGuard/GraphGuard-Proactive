...
if __name__ == '__main__':
    ...
    if not os.path.exists(f'{dataset}_{model}_auc_score.csv'):
        df = pd.DataFrame()
        ...
        df = pd.read_csv(f'{dataset}_{model}_auc_score.csv')
        prob_dict, log_path = get_prob_files_by_dataset(dataset, model)
        # get the original accuracy and mia attack accuracy
        acc_dict = get_acc_and_mia_acc(log_path, model)
        print(f"{dataset}_{model}_auc_score:\n {acc_dict} \n")